...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiRequest<{ user: User; accessToken: string }>('/auth/register', 'POST', { nome, email, telefone, senha });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#050505] relative">
      ...
      <div className="max-w-md w-full p-8 bg-[#0a0a0a] rounded-xl shadow-[0_0_50px_rgba(212,175,55,0.05)] border border-[#D4AF37]/20">
        <h2 className="text-3xl font-bold text-center text-[#D4AF37] mb-8">Cadastro EAD</h2>
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-300 mb-2">Telefone (WhatsApp)</label>
            <input
              type="text"
              value={telefone}
        ...
        <p className="mt-6 text-center text-sm text-neutral-400">
          Já possui conta? <Link href="/login" className="text-[#D4AF37] hover:underline font-medium">Faça Login</Link>
        </p>
      </div>
    </div>
  );
